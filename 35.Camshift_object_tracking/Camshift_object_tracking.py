...
# CamShift apply meanShift first then ones the meanShift converges it update the size of the window.
# In addition it also calculate the orientation of the best fit eclipse to it.
...
while(1):
    ...
    if ret == True:
        ...
        ret, track_window = cv.CamShift(dst, track_window, term_crit) # image, track image, termination criteria
        # CamShift function return rotated rectangle and also the box parameters
        # print(ret) # (x, y), width, height, rotation value

        # Draw it on image
        pts = cv.boxPoints(ret)
        print(pts)
        pts = np.int0(pts)
        final_image = cv.polylines(frame, [pts], True, (0, 255, 0), 2) # image, pts value, close/open, color, thickness
